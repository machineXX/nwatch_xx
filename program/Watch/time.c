...
void time_set(timeDate_s* newTimeDate) //rtc的保存方式 已经弃用
{
...
static void getRtcTime()
{
	     ...
	     rtc_get(&timeDate);
			...
			#else
	//	Read_Flash_Time_Data();
		if(!Read_Flash_Time_Data())
		{
			timeDate.date.year  =	'0'-28;
			timeDate.date.month =	(month_t)DEVICE_Init_time_MON;
			timeDate.date.day   =	(day_t)DEVICE_Init_time_WEEK;
			timeDate.date.date	=	DEVICE_Init_time_DAY;
			timeDate.time.hour	=	DEVICE_Init_time_HOUR;
			timeDate.time.mins	=	DEVICE_Init_time_MIN;
			timeDate.time.secs	=	0;
		}
